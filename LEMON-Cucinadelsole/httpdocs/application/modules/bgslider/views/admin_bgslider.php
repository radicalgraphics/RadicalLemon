										...
										<?php foreach ($active_bgslide as $active) : ?>
												<li id="<?php echo $active->id;?>" class="ui-state-default">
													<div class="image-area">
															...
															<div class="slide-bg-<?php echo $active->img; ?>">
																	<div class="slide-title"><?php echo $active->title; ?></div>
										...
										<?php foreach ($non_active_bgslide as $nonactive) : ?>
												<li id="<?php echo $nonactive->id;?>" class="ui-state-default">
													<div class="image-area">
														...
														<div class="slide-bg-<?php echo $nonactive->img; ?>">
																	<div class="slide-title"><?php echo $nonactive->title; ?></div>
...
$(document).ready(function() {  
			...
			receive: function( event, ui ) {
				...
				if (area == "sortable2") {				
						$.ajax({
								...
								type: 'POST',
								data: { "template": "bgslider", "method" : "setNonActiveSlide", "id_page_data": <?php echo $id; ?>, "video_id": video_id },
				...
				if ($("#sortable1 li").length <= 16) {
			
						if (area == "sortable1") {		
								$.ajax({
										url: "<?php echo site_url('admin/moduleFunctions');?>",
										type: 'POST',
										data: { "template": "bgslider", "method" : "setActiveSlide", "id_page_data": <?php echo $id; ?>, "video_id": video_id },
										success: function(data) {
													$('#imgstatus').html(data);
									   }
								});   	
					...
					$("#overlay").fadeIn().show().html('<div class="message"><div class="message-container"><div class="title">Message!</div><div class="top-line"></div><div class="message-icon"><img src="<?php echo base_url()."skin/admin/images/ls-navigation.png"?>"/></div><div class="message-text">You can publish only 16 slides!</div><div class="close"><button class="close-button" type="button" onclick="window.location.reload()"></button></div><div class="bottom-line"></div></div></div>');
			...
			$( '#sortable2 .image-area .over-slide-holder').html('<div></div>');
				...
				if (area == "sortable2") {		
						$.ajax({
								url: "<?php echo site_url('admin/moduleFunctions');?>",
								type: 'POST',
								data: { "template": "bgslider", "method" : "slideOrderNonActive", "id_page_data": <?php echo $id; ?>, "orderId": info },
								success: function(data) {
										$('#imgstatus').html(data);
								}
						});   
				...
				if (area == "sortable1") {		
						$.ajax({
								url: "<?php echo site_url('admin/moduleFunctions');?>",
								type: 'POST',
								data: { "template": "bgslider", "method" : "slideOrderActive", "id_page_data": <?php echo $id; ?>, "orderId": info },
								success: function(data) {
											$('#imgstatus').html(data);
								}
						});   
	...
	$('ul li').hover( 
		...
		var slide_id = $(this).attr('id');
			...
			$( '#sortable2 #'+slide_id+' .image-area .over-slide-holder').html('<div class="button-over-set"><button id="w'+slide_id+'" class="edit-over-button"></button><button id="t'+slide_id+'" class="title-over-button"></button><button id="e'+slide_id+'" class="event-over-button"></button><button id="u'+slide_id+'" class="attach-over-button"></button><button id="p'+slide_id+'" class="preview-over-button"></button><button id="d'+slide_id+'" class="delete-over-button"></button></div>');
			...
			$('#w'+slide_id).click(function(){
				...
				$("#overlay").show();
									...
									$.ajax({
												url: "<?php echo site_url('admin/moduleFunctions');?>",
											type: 'post',
											data: { "template": "bgslider", "method" : "writeSlide", "slide_id": slide_id  },
											success: function(data) {
			...
			$('#t'+slide_id).click(function(){
				...
				$("#overlay").show();
									...
									$.ajax({
												url: "<?php echo site_url('admin/moduleFunctions');?>",
											type: 'post',
											data: { "template": "bgslider", "method" : "addTitle", "slide_id": slide_id  },
											success: function(data) {
            ...
            $('#u'+slide_id).click(function(){
				...
				$("#overlay").show();
									...
									$.ajax({
												url: "<?php echo site_url('admin/moduleFunctions');?>",
											type: 'post',
											data: { "template": "bgslider", "method" : "uploadPdf", "slide_id": slide_id  },
											success: function(data) {
			...
			$('#e'+slide_id).click(function(){
				...
				$("#overlay").show();
									...
									$.ajax({
                                            url: "<?php echo site_url('admin/moduleFunctions');?>",
											type: 'post',
											data: { "template": "bgslider", "method" : "addDate", "slide_id": slide_id  },
											success: function(data) {
			...
			$("#overlay").show();
				  ...
				  $.ajax({
											url: "<?php echo site_url('admin/moduleFunctions');?>",
											type: 'post',
											data: { "template": "bgslider", "method" : "previewSlide", "video_id": slide_id  },
										   success: function(data) {
				...
				$("#overlay-content").show().html('<div class="message"><div class="message-container"><div class="title">Message!</div><div class="top-line"></div><div class="message-icon"><img src="<?php echo base_url()."skin/admin/images/ls-navigation.png"?>"/></div><div class="message-text">Do you want to delete this slide?</div><div class="close"><button id="del_no" class="no-button" type="button" onclick="window.location.reload()"></button><button id="del_yes" class="yes-button" type="button"></button></div><div class="bottom-line"></div></div></div>');
						...
						$("#del_yes").click(function() {
						   ...
						   $.ajax({
										url: "<?php echo site_url('admin/moduleFunctions');?>",
										type: 'post',
										data: { "template": "bgslider", "method" : "deleteSlide", "video_id": slide_id  },
										success: function(data) {
